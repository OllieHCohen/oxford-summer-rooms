...
| File | Purpose |
|---|---|
| `index.html` | **Properties** page: hero (incl. WhatsApp + blue "Reserve a room" buttons), trust pills, "How it works" + "What do I get?" modals, property cards (left) + **interactive Google map** (right). The card date badge ("Available: …") is **derived from the rooms** (earliest bookable start → latest end via `buildingAvailRange`), not the building's own `start_date`/`preceding_agreement_end`. |
